...
/**
 * ZupdatePlugin
 * 通过EventChannel与Flutter交互
 */
@TargetApi(Build.VERSION_CODES.M)
public class ZupdatePlugin implements FlutterPlugin, ActivityAware, EventChannel.StreamHandler, PluginRegistry.RequestPermissionsResultListener, ProgressListener {
  ...
  private static final String TAG = "FLUTTER ZUPDATE";
  private static final String EVENT_CHANNEL = "com.hacjy.zupdate";
  private static final String DEFAULT_APK_NAME = "z_update.apk";
  ...
  public static String apkDirectory ="/files/z_update";
  ...
  public static void registerWith(io.flutter.plugin.common.PluginRegistry.Registrar registrar) {
    ...
    ZupdatePlugin plugin = new ZupdatePlugin();
    plugin.initialize(registrar.context(), registrar.messenger());
    plugin.activity = registrar.activity();
    registrar.addRequestPermissionsResultListener(plugin);
  }

  public static void registerWithInstall(FlutterPluginBinding binding) {
    Log.d(TAG, "registerWithInstall");
    binding.getFlutterEngine().getPlugins().add(new InstallPlugin());
  }
  ...
  public void onListen(Object arguments, EventChannel.EventSink events) {
    if (progressSink != null) {
      progressSink.error("" + UpdateStatus.ALREADY_RUNNING_ERROR.ordinal(), "Method call was cancelled. One method call is already running!", null);
    }
    ...
    if (authority != null) {
      ...
      androidProviderAuthority = context.getPackageName() + "." + "z_update_provider";
    }

    int SKIP_WRITE_EXTERNAL_STORAGE_SDK_INT = 33;

    // WRITE_EXTERNAL_STORAGE permission always returns false on sdk 33
    boolean skipWriteExternalStorage = Build.VERSION.SDK_INT >= SKIP_WRITE_EXTERNAL_STORAGE_SDK_INT;
    ...
    if (requestCode == 0 && grantResults.length > 0) {
      for (int grantResult : grantResults) {
        if (grantResult != PackageManager.PERMISSION_GRANTED) {
          reportError(UpdateStatus.PERMISSION_NOT_GRANTED_ERROR, "Permission not granted", null);
          return false;
        }
      }
      executeDownload();
  ...
  private void executeDownload() {
    try {
      String dataDir = context.getApplicationInfo().dataDir + apkDirectory;
      //PREPARE URLS
      ...
      if (file.exists()) {
        ...
        if (!file.getParentFile().mkdirs()) {
          reportError(UpdateStatus.INTERNAL_ERROR, "unable to create ota_update folder in internal storage", null);
        }
      }

      Log.d(TAG, "DOWNLOAD STARTING");
      //一开始就通知在下载中，避免网络异常时，一直没有刷新进度，等待timeout，体验不好
      if(progressSink!=null) {
        progressSink.success(Arrays.asList("" + UpdateStatus.DOWNLOADING.ordinal(), "0"));
      }
      ...
      client.newCall(request.build()).enqueue(new Callback() {
        ...
        public void onFailure(@NotNull Call call, @NotNull IOException e) {
          reportError(UpdateStatus.DOWNLOAD_ERROR, e.getMessage(), e);
        }

        @Override
        public void onResponse(@NotNull Call call, @NotNull Response response) throws IOException {
          if (!response.isSuccessful()) {
            reportError(UpdateStatus.DOWNLOAD_ERROR, "Http request finished with status " + response.code(), null);
          }
          try {
            ...
            reportError(UpdateStatus.DOWNLOAD_ERROR, ex.getMessage(), ex);
            return;
          }
      ...
      reportError(UpdateStatus.INTERNAL_ERROR, e.getMessage(), e);
    }
  }
  ...
  private void onDownloadComplete(final String destination, final Uri fileUri) {
    ...
    if (!downloadedFile.exists()) {
      reportError(UpdateStatus.DOWNLOAD_ERROR, "File was not downloaded", null);
      return;
    }
    ...
    if (checksum != null) {
      ...
      try {
        if (!Sha256ChecksumValidator.validateChecksum(checksum, downloadedFile)) {
          ...
          reportError(UpdateStatus.CHECKSUM_ERROR, "Checksum verification failed", null);
          return;
        }
      } catch (RuntimeException ex) {
        //SEND CHECKSUM ERROR EVENT
        reportError(UpdateStatus.CHECKSUM_ERROR, ex.getMessage(), ex);
        return;
      }
    }
  ...
  private void executeInstallation(Uri fileUri, File downloadedFile) {
    ...
    if (progressSink != null) {
      ...
      progressSink.success(Arrays.asList("" + UpdateStatus.INSTALLING.ordinal(), ""));
   ...
   *
   * @param UpdateStatus Status to report
   * @param s         Error message to report
   */
  private void reportError(final UpdateStatus UpdateStatus, final String s, final Exception e) {
    if (Looper.getMainLooper().isCurrentThread()) {
      Log.e(TAG, "ERROR: " + s, e);
      if (progressSink != null) {
        progressSink.error("" + UpdateStatus.ordinal(), s, null);
        progressSink = null;
      }
    } else {
      ...
      public void handleMessage(Message msg) {
        ...
        if (progressSink != null) {
          ...
          if (data.containsKey(ERROR)) {
            reportError(UpdateStatus.DOWNLOAD_ERROR, data.getString(ERROR), null);
          } else {
            long bytesDownloaded = data.getLong(BYTES_DOWNLOADED);
            long bytesTotal = data.getLong(BYTES_TOTAL);
            progressSink.success(Arrays.asList("" + UpdateStatus.DOWNLOADING.ordinal(), "" + ((bytesDownloaded * 100) / bytesTotal)));
          }
        }
      }
    };
    final EventChannel progressChannel = new EventChannel(messanger, EVENT_CHANNEL);
            ...
            .addNetworkInterceptor(new Interceptor() {
              ...
              public Response intercept(@NotNull Chain chain) throws IOException {
                ...
                return originalResponse.newBuilder()
                        .body(new ProgressResponseBody(originalResponse.body(), ZupdatePlugin.this))
                        .build();
              }
            })
            .build();
