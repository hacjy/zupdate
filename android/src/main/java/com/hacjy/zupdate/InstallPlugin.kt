...
class InstallPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: Result) {
        when (call.method) {
            ...
            "getApkDirectory" -> {
                result.success(getApkPath()+ZupdatePlugin.apkDirectory);
            }
    ...
    private fun getApkPath():String{
        var applicationInfo = pluginBinding?.activity?.application?.applicationInfo
    ...
    private fun installApk(filePath: String?, currentAppId: String?) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            //直接安装，避免android11设置安装未知应用后不弹出安装应用弹窗
    ...
    private fun install24(context: Context?, file: File?, appId: String?) {
        ...
        val uri: Uri = FileProvider.getUriForFile(context, "$appId.z_update_provider", file)
        intent.setDataAndType(uri, "application/vnd.android.package-archive")
        context.startActivity(intent)
    }
